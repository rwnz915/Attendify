...
public class AttendanceFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new StudentAdapter(requireContext(), getFilteredList());
        adapter.setOnStudentClickListener(position -> {
            Student tapped = getFilteredList().get(position);
    ...
    private void applyFilter(String filter) {
        ...
        for (int i = 0; i < filters.length; i++) {
            if (labels[i].equals(filter)) {
                ...
                filters[i].setTextColor(requireContext().getResources().getColor(R.color.white, requireContext().getTheme()));
            } else {
                filters[i].setBackgroundResource(R.drawable.bg_filter_inactive);
                filters[i].setTextColor(requireContext().getResources().getColor(R.color.gray_600, requireContext().getTheme()));
            }
        }
