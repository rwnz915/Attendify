 ...
 *
 * Shows all subjects belonging to the secretary's section only.
 * Same design as SubjectFragment (teacher) but with a GREEN header instead of indigo,
 * and filtered to only the secretary's own section — no section chips needed.
 *
 * When a subject card is tapped, a bottom sheet lists the students in that section.
 */
public class SecretarySubjectFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        return inflater.inflate(R.layout.fragment_secretary_subject, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (tvSectionLabel != null)
            tvSectionLabel.setText(section != null ? "Section " + section : "My Section");

        loadSubjects(section);
    }

    // ── Load subjects for this secretary's section ────────────────────────────
    ...
    private void showEmpty() {
        if (tvTotalSubjects != null) tvTotalSubjects.setText("0");
        subjectContainer.removeAllViews();
        TextView tv = new TextView(requireContext());
        tv.setText("No subjects found for your section.");
    ...
    private View buildSubjectCard(SubjectItem subject, String color) {
        ...
        ((TextView) card.findViewById(R.id.tv_item_teacher)).setText(
                subject.teacher != null ? subject.teacher : "");
        ((TextView) card.findViewById(R.id.tv_item_schedule)).setText(
    ...
    private void showDetailSheet(SubjectItem subject, String color) {
        ...
        FrameLayout.LayoutParams clp = new FrameLayout.LayoutParams(dp(32), dp(32));
    ...
    // ── Render student rows in bottom sheet ───────────────────────────────────
    ...
    private static class MaxHeightFrameLayout extends FrameLayout {
